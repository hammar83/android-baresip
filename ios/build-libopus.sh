...
# Taken from https://github.com/chrisballinger/Opus-iOS
# Modified for Freespee purposes.
# Use env variables setup from generate_dist.sh
# Output in opus-build

if [ "$OPUS_VERSION" == "" ]; then
  echo "NO OPUS_VERSION env variable set"
  exit 1
fi
if [ "$MIN_IOS_VERSION" == "" ]; then
  echo "NO MIN_IOS_VERSION env variable set"
  exit 1
fi
...
do
	...
	mkdir -p "${INTERDIR}/${PLATFORM}-${ARCH}.sdk"

	./configure --enable-float-approx --disable-shared --enable-static --with-pic --disable-extra-programs --disable-doc ${EXTRA_CONFIG} \
    --prefix="${INTERDIR}/${PLATFORM}-${ARCH}.sdk" \
    LDFLAGS="$LDFLAGS ${OPT_LDFLAGS} -fPIE -miphoneos-version-min=${MINIOSVERSION} -L${OUTPUTDIR}/lib" \
    CFLAGS="$CFLAGS ${EXTRA_CFLAGS} ${OPT_CFLAGS} -fembed-bitcode -fPIE -miphoneos-version-min=${MINIOSVERSION} -I${OUTPUTDIR}/include -isysroot ${DEVELOPER}/Platforms/${PLATFORM}.platform/Developer/SDKs/${PLATFORM}.sdk" \
...
for OUTPUT_LIB in ${OUTPUT_LIBS}; do
	...
	for ARCH in ${ARCHS}; do
		...
		INPUT_ARCH_LIB="${INTERDIR}/${PLATFORM}-${ARCH}.sdk/lib/${OUTPUT_LIB}"
...
for ARCH in ${ARCHS}; do
	...
	cp -R ${INTERDIR}/${PLATFORM}-${ARCH}.sdk/include/* ${OUTPUTDIR}/include/
	if [ $? == "0" ]; then
